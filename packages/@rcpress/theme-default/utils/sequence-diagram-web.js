(function () {
	...
	class BasicTheme extends BaseTheme {
		constructor(svg) {
			...
			this.addConnectLine('solid', {
				attrs: {
					'stroke': '#000000',
					'stroke-width': 1,
				}
			});
			this.addConnectLine('dash', {
				attrs: {
					'stroke-dasharray': '4, 2',
				}
			});
	...
	const makeCommands = ((() => {
		...
		const refDef = {
			type: 'keyword', then: Object.assign({
				'over': {
					type: 'keyword', then: {
		...
		const divider = {
			...
			'with': {
				type: 'keyword', suggest: ['with height '], then: {
					'height': {
						type: 'keyword', then: {
							'': {
		...
		const BASE_THEN = {
			...
			'if': commonGroup,
			'else': {
				type: 'keyword', suggest: ['else\n', 'else if: '], then: {
					'if': {
						type: 'keyword', suggest: ['if: '], then: {
							'': textToEnd,
							':': {
								type: 'operator', then: {
									'': textToEnd,
			...
			'state': {
				type: 'keyword', suggest: ['state over '], then: {
					'over': {
						type: 'keyword', then: {
							'': {
								type: 'variable',
